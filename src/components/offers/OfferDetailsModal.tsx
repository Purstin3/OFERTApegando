...
const OfferDetailsModal: React.FC<OfferDetailsModalProps> = ({
  ...
  const stats = React.useMemo(() => {
    ...
    const avg = adCounts.length > 0 
      ? Math.round(adCounts.reduce((sum, c) => sum + c.count, 0) / adCounts.length)
      : 0;
  ...
  const platform = platformInfo[offer.platform] || platformInfo.other;
